...
import mutagen.id3  # type: ignore[import-untyped]
from mutagen.id3 import ID3, TIT2, TPE1, TALB, TDRC, TCON, TRCK, APIC, USLT  # type: ignore[import-untyped]
from mutagen.mp4 import MP4, MP4Cover  # type: ignore[import-untyped]
from mutagen.flac import FLAC, Picture  # type: ignore[import-untyped]
...
def tag_file(filepath, meta, cover_data=None):
    ...
    tag_fn = taggers.get(ext)
    if tag_fn is None:
        raise ValueError(f"Unsupported format: {ext}")
    try:
        tag_fn(filepath, meta, cover_data)
    except (mutagen.MutagenError, OSError, RuntimeError) as e:  # type: ignore
        raise RuntimeError(f"Failed to tag {ext} file: {e}") from e


def _tag_mp3(filepath, meta, cover_data):
    try:
        audio = ID3(filepath)
    except mutagen.id3.ID3NoHeaderError:  # type: ignore
        audio = ID3()
